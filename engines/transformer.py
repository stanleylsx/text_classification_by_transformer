...
class MultiHeadAttention:
    def __init__(self, embedding_dim, dropout_rate):
        ...
        self.W_Q = tf.keras.layers.Dense(embedding_dim, use_bias=False)
        self.W_K = tf.keras.layers.Dense(embedding_dim, use_bias=False)
        self.W_V = tf.keras.layers.Dense(embedding_dim, use_bias=False)
        self.W_O = tf.keras.layers.Dense(embedding_dim, use_bias=False)
...
class FeedForward:
    def __init__(self, embedding_dim, dropout_rate):
        ...
        self.dense2 = tf.keras.layers.Dense(embedding_dim, use_bias=False)
        self.dropout = tf.keras.layers.Dropout(dropout_rate)
        self.layer_norm = tf.keras.layers.LayerNormalization(epsilon=1e-6)
